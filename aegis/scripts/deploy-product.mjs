// 部署升级后的 ReceiptRegistry 并用 D4E2E 真 quote 提交一张收据
...
console.log("submit tx:", tx.hash, "status:", r.status, "gas:", r.gasUsed.toString());
console.log("lastReceiptHash:", await reg.lastReceiptHash(1));
const rec = await reg.latestReceipt(1);
console.log("stored executionHash:", rec.executionHash);
console.log("stored nonce:", rec.nonce);
console.log("stored quoteHash:", rec.quoteHash);
console.log("\nNEW_REGISTRY=" + addr);
